...
Go sidecar. React invokes only fixed Core lifecycle, Endpoint, local
...
The Core binds inference to `127.0.0.1:8317` by default and chooses an ephemeral
loopback control port. It writes exactly one `ready` JSON line to stdout. Rust
...
reports Core as ready. An unexpected exit is surfaced to the UI; M0 deliberately
provides a manual restart entry point but does not claim complete crash recovery.
...
Profiles are UI-only templates. Existing Endpoint records are hydrated from the
