...
export type ModelTestState = "queued" | "running" | "success" | "failed" | "stopped";
...
export async function runModelTestBatch({ models, input, concurrency, shouldStop, onUpdate, test }: {
  models: string[];
  ...
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 3 || models.length > MAX_BATCH_MODELS || new Set(models).size !== models.length) {
  ...
  async function worker() {
    while (!shouldStop() && index < targets.length) {
      ...
      try {
        ...
        onUpdate({ model, state: "failed", error: error instanceof Error ? error.message : String(error) });
      }
    }
  }
  await Promise.all(Array.from({ length: Math.min(concurrency, targets.length) }, worker));
  for (; index < targets.length; index++) onUpdate({ model: targets[index], state: "stopped" });
}
