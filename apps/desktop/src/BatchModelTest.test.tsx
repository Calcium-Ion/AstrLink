...
const service: Service = {
  id: "service_test", name: "Batch provider", kind: "openai", enabled: true,
  models: ["alpha", "beta", "gamma"], capabilities: [{ protocol: "openai.chat", mode: "native", streaming: true }],
...
const result = (model: string, ok = true): ServiceTestResult => ({
  service_id: service.id, protocol: "openai.chat", stream: true, model, ok, status_code: ok ? 200 : 429,
  duration_ms: 1250, response_headers_ms: 100, first_token_ms: ok ? 230 : null,
  output: ok ? "**OK**" : "", raw_response: ok ? 'data: {"text":"OK"}\n\ndata: [DONE]\n\n' : '{"error":{"message":"quota"}}',
  response_content_type: ok ? "text/event-stream" : "application/json", ...(ok ? {} : { message: "quota", error_code: "upstream_error" }),
});
...
function button(text: string) {
  ...
  const match = buttons().find(item => (item.getAttribute("aria-label") ?? item.textContent) === text && !item.closest(".hidden"));
  if (!match) throw new Error(`Missing button ${text}`);
  return match;
}
const batch = () => document.querySelector<HTMLElement>('[data-testid="batch-model-tests"]')!;
const modelRow = (model: string) => [...batch().querySelectorAll("tbody tr")].find(row => row.querySelector("[title]")?.getAttribute("title") === model)!;
...
async function search(value: string) {
  const input = batch().querySelector<HTMLInputElement>('input[type="search"]')!;
  await act(async () => {
...
it("runs selected models, shows original failures and retries only failed items", async () => {
  mocks.testService.mockImplementation(async (_id: string, input: ServiceTestInput) => result(input.model, input.model !== "beta"));
  await open();
  await act(async () => modelRow("gamma").querySelector<HTMLButtonElement>('[role="checkbox"]')!.click());
  await act(async () => button("测试 2 个模型").click());
  expect(mocks.testService.mock.calls.map(([, input]) => input.model)).toEqual(["alpha", "beta"]);
  expect(modelRow("alpha").textContent).toContain("成功0.23 s1.25 s");
  expect(modelRow("beta").textContent).toContain("失败—1.25 s");
  await act(async () => modelRow("beta").querySelector<HTMLButtonElement>('[aria-label="查看 beta 的测试响应"]')!.click());
  await act(async () => button("原文").click());
  expect(batch().querySelector("pre")?.textContent).toBe(result("beta", false).raw_response);
  await act(async () => button("返回模型列表").click());
  mocks.testService.mockImplementation(async (_id: string, input: ServiceTestInput) => result(input.model));
  await act(async () => button("重试失败 (1)").click());
  expect(mocks.testService.mock.calls.map(([, input]) => input.model)).toEqual(["alpha", "beta", "beta"]);
...
it("blocks mode switches and close until active tests finish, and stops pending models", async () => {
  ...
  mocks.testService.mockImplementation((_id: string, input: ServiceTestInput) => new Promise<ServiceTestResult>(resolve => pending.set(input.model, resolve)));
  await open();
  ...
  await act(async () => { pending.get("alpha")!(result("alpha")); pending.get("beta")!(result("beta")); });
  expect(mocks.testService).toHaveBeenCalledTimes(2);
...
it("selects matching models and adds test-only models without changing the provider", async () => {
  ...
  await search("alpha");
  await act(async () => batch().querySelector<HTMLButtonElement>('[aria-label="选择全部匹配模型"]')!.click());
  await search("custom-model");
  await act(async () => button("添加模型").click());
  await act(async () => button("测试 2 个模型").click());
  expect(mocks.testService.mock.calls.map(([, input]) => input.model)).toEqual(["alpha", "custom-model"]);
...
it("limits the initial selection to 100 models and accepts unusual upstream IDs", async () => {
  const models = ["toString", "__proto__", ...Array.from({ length: 99 }, (_, index) => `model-${index}`)];
  await open(models);
  expect(button("测试 100 个模型").disabled).toBe(false);
  expect(modelRow("model-98").querySelector<HTMLButtonElement>('[role="checkbox"]')!.disabled).toBe(true);
