import type { PrivacyDryRunFinding, PrivacyDryRunProtocol } from "./privacy-policy-model";
...
export function dryRunSampleText(body: string, protocol: PrivacyDryRunProtocol): string | null {
...
export function dryRunFindingSpan(body: string, finding: PrivacyDryRunFinding): DryRunTextSpan | null {
...
export function createDryRunFindingResolver(body: string) {
  ...
  try { document = JSON.parse(body); } catch { document = null; }
  ...
  return (finding: PrivacyDryRunFinding): DryRunTextSpan | null => {
    if (!fields.has(finding.path)) {
      ...
      fields.set(finding.path, text === null ? null : { text, bytes: new TextEncoder().encode(text) });
    }
    ...
    try {
      ...
      return { text, value, start: prefix.length, end: prefix.length + value.length };
    } catch {
      return null;
    }
  };
