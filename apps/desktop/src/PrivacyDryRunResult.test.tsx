...
describe("privacy dry-run explanations", () => {
  ...
  const finding = { kind: "email" as const, path: "/input", start: 0, end: sample.length, confidence: 0.99 };
  const base: CompletedPrivacyDryRun = {
    protocol: "openai.responses", detector: "local_model", minConfidence: 0.6,
    decision: "allow", findings_summary: "", findings: [], suppressed_findings: [],
    inspected_body: JSON.stringify({ input: sample }),
  };
  const render = async (result: CompletedPrivacyDryRun) => act(async () => {
    root.render(<PrivacyDryRunResult result={result} summary="" sample={sample} onLocate={vi.fn()} />);
  });

  it("shows the original for every suppression cause without confusing policy exclusions with low confidence", async () => {
    await render({ ...base, suppressed_findings: [
      { ...finding, reason: "low_confidence", confidence: 0.5999 },
      ...(["kind_disabled", "allowlisted", "placeholder", "unrepresentable"] as const).map((reason) => ({ ...finding, reason })),
    ] });
    const rows = [...container.querySelectorAll('[data-testid="dry-run-finding"]')];
    expect(rows.map((row) => row.querySelector("mark")?.textContent)).toEqual(Array(5).fill(sample));
    ...
    for (const row of rows.slice(1)) expect(row.textContent).not.toContain("门槛");
  ...
  it("matches replacements by both kind and original text", async () => {
    await render({ ...base, decision: "redact", findings: [finding], redactions: [
      { kind: "phone", value: sample, placeholder: "wrong-kind", style: "token" },
      { kind: "email", value: "bob@example.com", placeholder: "wrong-value", style: "token" },
      { kind: "email", value: sample, placeholder: "replacement@example.invalid", style: "natural" },
    ...
    expect(container.querySelector('[aria-label="处理后文本"]')?.textContent).toContain("replacement@example.invalid");
  });
});
