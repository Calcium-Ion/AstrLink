...
const components: Components = {
  ...
  ol: ({ children, start }) => <ol className="list-decimal space-y-1 pl-5" start={start}>{children}</ol>,
  ...
  pre: ({ children }) => <pre className="max-w-full overflow-x-auto rounded-md border bg-background p-3 text-xs [&>code]:bg-transparent [&>code]:p-0">{children}</pre>,
  code: ({ children, className }) => <code className={`rounded-sm bg-background px-1 font-mono text-xs ${className ?? ""}`}>{children}</code>,
  ...
  img: ({ alt, src }) => <span className="text-muted-foreground">{alt || src}</span>,
  input: ({ checked }) => <Checkbox checked={checked ?? false} disabled className="mr-1 inline-flex align-middle" />,
