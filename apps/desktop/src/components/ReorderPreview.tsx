...
export function ReorderPreview({ items, order, lifted, firstLabel, label }: {
  ...
  return (
    <div className="relative min-w-0 py-2">
      ...
      />
      <ol aria-label={label} className="relative m-0 list-none p-0" style={{ height: items.length * rowHeight }}>
        {order.map((id, index) => {
          ...
          return (
            <motion.li
              key={id}
              initial={false}
              animate={{ y: index * rowHeight, x: dragging ? 8 : 0, scale: dragging ? 1.015 : 1 }}
              ...
              className={cn(
                ...
                dragging ? "z-20 bg-card shadow-lg ring-1 ring-primary/60" : "bg-card/70",
              )}
            ...
            >
              <span className="relative flex size-6 shrink-0 items-center justify-center text-muted-foreground" aria-hidden="true">
                <motion.span
                  initial={false}
                  animate={{ opacity: dragging ? 1 : 0, scale: dragging ? 1 : 0.6 }}
                  transition={{ duration: reducedMotion ? 0 : 0.2 }}
                  className="absolute -inset-1 rounded-full border-2 border-primary bg-primary/10"
                />
                <GripVertical className={cn("size-4", dragging && "text-primary")} />
                {dragging ? (
                  <svg viewBox="0 0 24 24" className="absolute top-3 left-3 size-6 fill-primary stroke-primary-foreground drop-shadow-sm" strokeWidth="1.5">
              ...
              <span aria-hidden={index !== 0 || undefined} className={cn("shrink-0 text-xs font-medium text-primary", index !== 0 && "invisible")}>
                {firstLabel}
              </span>
            </motion.li>
          );
