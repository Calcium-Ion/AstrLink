...
const variants = [
  { code: "2A", name: "OPEN", file: "2a-open.svg", rx: 152, ry: 45, innerRx: 132, innerRy: 21, innerY: -3, angle: -13, cy: 280, leg: 44 },
  { code: "2B", name: "ASCEND", file: "2b-ascend.svg", rx: 154, ry: 43, innerRx: 134, innerRy: 20, innerY: -4, angle: -27, cy: 264, leg: 44 },
  { code: "2C", name: "FLOW", file: "2c-flow.svg", wave: true, leg: 44 },
  { code: "2D", name: "COMPACT", file: "2d-compact.svg", rx: 133, ry: 54, innerRx: 110, innerRy: 24, innerY: -5, angle: -20, cy: 274, leg: 48 },
];
...
function ellipseRing(v) {
  ...
  return { full, front, transform: `translate(256 ${v.cy}) rotate(${v.angle})` };
}
function waveRing() {
  return {
    ...
    front: "M111 286C101 303 103 317 112 327C136 353 224 333 295 307C377 278 430 236 397 211L383 229C391 239 349 266 287 288C220 312 158 325 136 311C130 308 131 301 134 292Z",
...
for (const v of variants) {
  ...
  const gradientAxis = v.wave ? 'x1="108" y1="318" x2="403" y2="224"' : `x1="${-v.rx}" y1="0" x2="${v.rx}" y2="0"`;
  const defs = `<defs>
...
const original = asImage(readFileSync(join(dir, "../explorations-ribbon/02-loop.svg")));
const cards = marks.map((v, index) => `<g transform="translate(${48 + index % 2 * 544} ${132 + Math.floor(index / 2) * 422})">
...
writeFileSync(join(dir, "comparison.svg"), `<svg xmlns="http://www.w3.org/2000/svg" width="1120" height="980" viewBox="0 0 1120 980">
