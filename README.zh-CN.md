...
- **模型路由**：将请求模型名重定向到目标模型，配置提供商优先级、失败重试和提供商切换。
...
| Coding Plan 订阅                                                         | 选择对应 Coding Plan，使用订阅专用凭据                               |
...
| Model    | API 提供商模型列表中的模型 ID，或已启用重定向规则中的请求模型名 |
...
- 在 **路由** 中配置模型重定向、失败重试和提供商切换。在 **API 提供商**
  列表中拖动调整优先级；网关会选择支持目标模型和请求协议的提供商。
- 在 **安全策略**
...
检查提供商是否启用、模型列表是否包含该模型，以及入口协议是否匹配。使用模型重定向时，还需检查规则是否启用，以及是否有提供商支持目标模型。
