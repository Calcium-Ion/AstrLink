...
把你的 AI 订阅和 API 服务接入一个本地网关，让 IDE、命令行工具和 AI Agent 共用模型、路由与隐私设置。

AstrLink 是一款开源桌面应用。在界面中添加服务、选择模型，再把客户端连接到本机 API 地址即可使用。支持 macOS、Windows 和 Linux。

[开始使用](#开始使用) · [服务接入指南](docs/guides/pay-as-you-go-providers.md) · [桌面设置](apps/desktop/README.md) · [参与开发](CONTRIBUTING.md)
...
- **排查调用问题**：查看请求记录、会话轨迹、上游尝试和错误原因，了解请求实际走向了哪个服务和模型。
- **查看使用情况**：按服务、模型和客户端令牌查看用量及费用估算。估算结果不替代服务商账单。

网关和隐私检测运行在本机；推理请求仍会发送到你配置的上游服务。模型是否可用、额度和计费取决于对应账号或 API 服务。
...
打开 AstrLink，确认网关已启动，然后进入 **API 服务 → 添加服务**。
...
保存后检查服务的模型列表，拉取或手动添加需要使用的模型。**模型列表为空的服务不会处理推理请求。** API 和 Coding Plan 的密钥、地址可能不同，详细说明见 [服务接入指南](docs/guides/pay-as-you-go-providers.md)。

### 2. 创建客户端令牌

在 **访问令牌** 中为客户端创建一个令牌。客户端填写的是 AstrLink 令牌；服务商的 API Key 保存在「API 服务」中。
...
服务必须支持客户端使用的接口格式，或配置可用的协议转换。连接成功后，在 **请求记录** 中确认请求结果。

### 4. 按需设置路由和隐私策略

- 在 **路由** 中配置模型别名、目标服务和重试行为。使用 `astrlink/auto` 前，先完成自动路由的分类模型和目标配置。
...
确认填写的是仍然有效的 AstrLink 访问令牌。如果请求记录显示上游返回 401 或 403，再检查对应服务的密钥或订阅登录状态。

**找不到模型或没有可用服务？**

检查服务是否启用、模型列表是否包含该模型，以及入口协议是否匹配。使用模型别名时，还需检查路由目标。
